...
# Create a 3D material with certain properties
material = Material3D(shape='homogeneous', metal=False)
material.setup()
material._calculate_n()

# Create a 3D source with specified properties
...
def em3D(mx=64, my=64, mz=64, num_frames=10, cfl=1.0, outdir='./_output', before_step=False, debug=False, nl=False, psi=True):
    ...
    # Print material and source information if in debug mode and running on rank 0
    ...
    # Configure before step if necessary
    if before_step:
    ...
    # Array initialization for source and material
    source.init(state)
    material.init(state)
