...
# Define the boundaries of the simulation domain
x_lower = 0.0
x_upper = 50.0
y_lower = 0.0
y_upper = 50.0
...
# Create a 2D material with specific properties
material = Material2D(shape='homogeneous', metal=False)
material.setup()
material._calculate_n()

# Define a 2D electromagnetic simulation function
def em2D(mx=128, my=128, num_frames=10, cfl=1.0, outdir='./_output', before_step=False, debug=False,
         heading='x', shape='off', nl=False, psi=True, conservative=True):
    import clawpack.petclaw as pyclaw
    import petsc4py.PETSc as MPI

    # Create a 2D source object
    ...
    _, _, dt, tf = basics.grid_basic([[x_lower, x_upper, mx], [y_lower, y_upper, my]], cfl=cfl, co=material.co, v=source.v)

    # Display material and source information if in debug mode
    ...
    # Create dimensions and domain for the simulation
    ...
    # Configure before-step callback if needed
    if before_step:
    ...
    # Initialize source and material arrays
    source.init(state)
    material.init(state)

    # If in conservative mode, scale the state array
    if conservative:
